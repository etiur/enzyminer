...
def fit():
    ...
    model_dict = {20: SVC(kernel="rbf", C=0.31, gamma=1), 40: SVC(kernel="rbf", C=0.61, gamma=1),
                  70: SVC(kernel="rbf", C=0.91, gamma=0.61), 80: SVC(kernel="rbf", C=5, gamma=0.31),
                  90: SVC(kernel="rbf", C=0.61, gamma=0.91)}
...
def get_scores(Y_test_svc, Y_train_svc, test_pred, train_pred):
    ...
    dataframe_20, te_report_20, tr_report_20 = to_dataframe(individual_scores[0], ["svc_20"])
    dataframe_40, te_report_40, tr_report_40 = to_dataframe(individual_scores[1], ["svc_40"])
    dataframe_70, te_report_70, tr_report_70 = to_dataframe(individual_scores[2], ["svc_70"])
    dataframe_80, te_report_80, tr_report_80 = to_dataframe(individual_scores[3], ["svc_80"])
    dataframe_90, te_report_90, tr_report_90 = to_dataframe(individual_scores[4], ["svc_90"])
...
def writing(dataset1, te_report, tr_report, sheet_name, row=0):
    """Writes to excel"""
    if not path.exists(f"ensemble_scores/intramodel_svc.xlsx"):
        wb = Workbook()
        ws = wb.active
        ws.title = sheet_name
        wb.save(filename=f"ensemble_scores/intramodel_svc.xlsx")

    book = load_workbook('ensemble_scores/intramodel_svc.xlsx')
    writer = pd.ExcelWriter('ensemble_scores/intramodel_svc.xlsx', engine='openpyxl')
